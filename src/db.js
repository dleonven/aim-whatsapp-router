...
function getActiveAgents() {
  return db.prepare('SELECT * FROM agents WHERE active = 1 ORDER BY last_assigned_at ASC NULLS FIRST').all();
}
...
function addAgent(name, waNumber, phoneNumberId = null) {
  const stmt = db.prepare('INSERT INTO agents (name, wa_number, phone_number_id) VALUES (?, ?, ?)');
  return stmt.run(name, waNumber, phoneNumberId);
}

function updateAgentLastAssigned(agentId) {
  const stmt = db.prepare('UPDATE agents SET last_assigned_at = CURRENT_TIMESTAMP WHERE id = ?');
...
function getAssignmentByLeadPhone(leadPhone) {
  return db.prepare('SELECT * FROM assignments WHERE lead_phone = ? ORDER BY created_at DESC LIMIT 1').get(leadPhone);
}

function createAssignment(leadPhone, leadName, agentId, messageText) {
  const stmt = db.prepare('INSERT INTO assignments (lead_phone, lead_name, agent_id, message_text) VALUES (?, ?, ?, ?)');
